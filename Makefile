...
#     ABSTRACT => q[fetch a remote image into a L<File::Temp>]
#     AUTHOR => [q[Mark Ellis <markellis@cpan.org>]]
#     BUILD_REQUIRES => { ExtUtils::MakeMaker=>q[6.59], Test::More=>q[0], Test::Exception=>q[0] }
#     CONFIGURE_REQUIRES => {  }
#     DISTNAME => q[Fetch-Image]
#     LICENSE => q[perl]
...
PARENT_NAME = Fetch
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Fetch/Image.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Fetch/Image.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Fetch
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Fetch
...
TO_INST_PM = lib/Fetch/Image.pm

PM_TO_BLIB = lib/Fetch/Image.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Fetch/Image.pm $(INST_MAN3DIR)/Fetch::Image.$(MAN3EXT) 
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>fetch a remote image into a L&lt;File::Temp&gt;</ABSTRACT>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Data::Validate::Image" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Fetch/Image.pm blib/lib/Fetch/Image.pm 
